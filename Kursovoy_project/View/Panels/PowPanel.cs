...
{
    public partial class PowPanel : UserControl
    {
        public PowPanel()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    k = Convert.ToInt32(richTextBox1.Text);
                    IsNum = true;
                }
                catch (Exception) { MessageBox.Show("Введите число типа int", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error); }

                if (IsNum)
                {
                    if (k < 1)
                    {
                        MessageBox.Show("Степень должна быть больше ил равна 1", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    else
                    {
                        if (Parent is MainWindow)
                        {
                            try
                            {
                                ...
                                ans = ((MainWindow)Parent).controller.pow(mat1.table, k);
